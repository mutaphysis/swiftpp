...
void <{class_name}>_proxy_init(const void *i_cxxptr, const void *i_self<{#virtual_methods:prefix(,\n     )separator(,\n     )}><{return_c_type}> (*i_<{name}>_callback)(const void *<{#params:prefix(, )separator(, )}><{param_c_type}><{/params}>)<{/virtual_methods}>);
<{#methods}>
<{return_c_type}> <{class_name}>_proxy_<{name}>(const void *i_cxxptr<{#params:prefix(, )separator(, )}><{param_c_type}> <{param_name}><{/params}>);
<{/methods}><{#static_methods}>
<{return_c_type}> <{class_name}>_proxy_<{name}>(<{#params:separator(, )}><{param_c_type}> <{param_name}><{/params}>);
...
<{#virtual_methods}>
  ...
  {
    (*_<{name}>_callback)(_self<{#params:prefix(, )separator(, )}><{param_as_c_type}><{/params}>);
  }
<{/virtual_methods}>
<{#methods}>
...
<{/methods}>
<{#static_methods}>
  inline static <{return_cxx_type}> <{name}>_forward(<{#params:separator(, )}><{param_cxx_type}> <{param_name}><{/params}>)
  {
    <{class_name}>::<{name}>(<{#params:separator(, )}><{param_name}><{/params}>);
  }
<{/static_methods}>
...
void <{class_name}>_proxy_init(const void *i_cxxptr, const void *i_self<{#virtual_methods:prefix(, )separator(, )}><{return_c_type}> (*i_<{name}>_callback)(const void *<{#params:prefix(, )separator(, )}><{param_c_type}><{/params}>)<{/virtual_methods}>)
{
...
<{return_c_type}> <{class_name}>_proxy_<{name}>(<{#params:separator(, )}><{param_c_type}> <{param_name}><{/params}>)
{
  <{class_name}>_proxy_class::<{name}>_forward(<{#params:separator(, )}><{param_as_cxx_type}><{/params}>);
}
<{/static_methods}>
#ifdef __cplusplus
}
...
<{#methods}>
  ...
  {
    <{class_name}>_proxy_<{name}>(self._super<{#params:prefix(, )separator(, )}><{param_clean_name}><{/params}>)
  }
<{/methods}>
<{#static_methods}>
  static func <{name}>(<{#params:separator(, )}><{param_clean_name}>: <{param_swift_type}><{/params}>)
  {
    <{class_name}>_proxy_<{name}>(<{#params:separator(, )}><{param_clean_name}><{/params}>)
  }
<{/static_methods}>
}<{/classes}>
)";
